...
export default function Analyze() {
    ...
    const { api, user, refreshUser } = useApp();
    ...
    const handleAnalyze = async (submitUrl?: string) => {
        ...
        if(user?.plan === "free" && (user.analysisCount || 0) >= 5) {
            setError("Free plan limit reached. Upgrade to Pro for unlimited scans.");
            toast.error("Free plan limit reached. Upgrade to Pro for unlimited scans.");
            return;
        }
